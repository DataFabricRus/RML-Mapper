...
public class StdRMLMappingExtractor implements RMLMappingExtractor {
    ...
    public Map<Resource, TriplesMap> extractTriplesMapResources(Repository repo) {
        return initializeTripleMapsResources(getTriplesMapLogicalSources(repo));
    }

    /**
     * @return
     */
    protected RepositoryResult<Statement> getTriplesMapLogicalSources(Repository repo) {
        RepositoryResult<Statement> statements = null;

        try {
            RepositoryConnection connection = repo.getConnection();

            //IRI o = vf.createIRI(R2RMLVocabulary.R2RML_NAMESPACE + R2RMLVocabulary.R2RMLTerm.TRIPLES_MAP_CLASS);
            IRI predicate = vf.createIRI(RMLVocabulary.RML_NAMESPACE + RMLVocabulary.RMLTerm.LOGICAL_SOURCE);

            statements = connection.getStatements(null, predicate, null, true);
     ...
     * @return
     */
    protected Map<Resource, TriplesMap> initializeTripleMapsResources(RepositoryResult<Statement> statements) {
        Map<Resource, TriplesMap> triplesMapResources = new HashMap<>();
        try {
    ...
    public Repository replaceShortcuts(Repository mapDocRepo) {
        ...
        Map<IRI, IRI> shortcutPredicates = new HashMap<>();
        RepositoryConnection mapDocRepoCon = null;
        try {
            mapDocRepoCon = mapDocRepo.getConnection();

            shortcutPredicates.put(
                    vf.createIRI(R2RMLVocabulary.R2RML_NAMESPACE + R2RMLVocabulary.R2RMLTerm.SUBJECT),
                    vf.createIRI(R2RMLVocabulary.R2RML_NAMESPACE + R2RMLVocabulary.R2RMLTerm.SUBJECT_MAP)
            );

            shortcutPredicates.put(
                    vf.createIRI(R2RMLVocabulary.R2RML_NAMESPACE + R2RMLVocabulary.R2RMLTerm.PREDICATE),
                    vf.createIRI(R2RMLVocabulary.R2RML_NAMESPACE + R2RMLVocabulary.R2RMLTerm.PREDICATE_MAP)
            );

            shortcutPredicates.put(
                    vf.createIRI(R2RMLVocabulary.R2RML_NAMESPACE + R2RMLVocabulary.R2RMLTerm.OBJECT),
                    vf.createIRI(R2RMLVocabulary.R2RML_NAMESPACE + R2RMLVocabulary.R2RMLTerm.OBJECT_MAP)
            );

            shortcutPredicates.put(
                    vf.createIRI(R2RMLVocabulary.R2RML_NAMESPACE + R2RMLVocabulary.R2RMLTerm.GRAPH),
                    vf.createIRI(R2RMLVocabulary.R2RML_NAMESPACE + R2RMLVocabulary.R2RMLTerm.GRAPH_MAP)
            );
            ...
            for (IRI uri : shortcutPredicates.keySet()) {
                ...
                while (shortcuts.hasNext()) {
                    ...
                    IRI pConstant = vf.createIRI(
                            R2RMLVocabulary.R2RML_NAMESPACE + R2RMLVocabulary.R2RMLTerm.CONSTANT
                    );
