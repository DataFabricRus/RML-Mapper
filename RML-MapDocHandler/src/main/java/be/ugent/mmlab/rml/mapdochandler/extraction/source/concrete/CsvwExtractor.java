...
public class CsvwExtractor extends StdSourceExtractor {
    ...
    public Set<Source> extractSources(Repository repository, Value value) {
        ...
        try {
            ...
            RepositoryResult<Statement> statements = extractURL(repository, (Resource) value);
            if (statements.hasNext()) {
                Statement statement = statements.next();
                Source source = extractSource(statement.getObject());
                sources.add(source);
            } else {
    ...
    public Source extractSource(Value value) {
        ...
        Source source = new StdApiSource(value.stringValue(), value.stringValue());
        return source;
    }
