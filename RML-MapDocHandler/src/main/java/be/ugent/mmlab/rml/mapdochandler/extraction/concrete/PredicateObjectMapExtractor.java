...
public class PredicateObjectMapExtractor {
    ...
    // Log
    static final Logger log = LoggerFactory.getLogger(PredicateObjectMapExtractor.class.getSimpleName());
    ...
    public PredicateObjectMap extractPredicateObjectMap(
        ...
        Set<PredicateMap> predicateMaps = new HashSet<>();
        Set<ObjectMap> objectMaps = new HashSet<>();
        Set<ReferencingObjectMap> refObjectMaps;
        Set<FunctionTermMap> funObjectMaps = new HashSet<>();

        try {
            RepositoryConnection connection = repository.getConnection();
            ValueFactory vf = connection.getValueFactory();

            IRI predicate = vf.createIRI(R2RMLVocabulary.R2RML_NAMESPACE + R2RMLVocabulary.R2RMLTerm.PREDICATE_MAP);
            predicate_statements = connection.getStatements(predicateObject, predicate, null, true);
            log.debug("More Predicate Map statements: {}", predicate_statements.hasNext());
            while (predicate_statements.hasNext()) {
                ...
                IRI o = vf.createIRI(R2RMLVocabulary.R2RML_NAMESPACE + R2RMLVocabulary.R2RMLTerm.OBJECT_MAP);
                // Extract object maps
                    ...
                    while (object_statements.hasNext()) {
                        ...
                        if (refObjectMaps.isEmpty()) {
                            ...
                            funObjectMaps = funObjMapExtractor.processFunctionTermMap(
                                    repository,
                                    (Resource) object_statement.getObject(),
                                    triplesMapResources,
                                    predicateObjectMap,
                                    savedGraphMap
                            );
                        ...
                        if (refObjectMaps.isEmpty() && funObjectMaps.isEmpty()) {
                            ...
                            ObjectMap objectMap = objMapExtractor.extractObjectMap(
                                    repository,
                                    (Resource) object_statement.getObject(),
                                    savedGraphMap,
                                    triplesMap,
                        ...
                        predicateObjectMap = new StdPredicateObjectMap(predicateMaps, objectMaps, refObjectMaps);
                    ...
                    /*predicateObjectMap = new StdPredicateObjectMap(
                        ...
                        if (predicateObjectMap != null) {
                            ...
                            try {
                                ...
                                RepositoryResult<Statement> dcTermsStatements =
                                        dcTermsConnection.getStatements(
                                                predicateObject,
                                                repository.getValueFactory().createIRI("http://purl.org/dc/terms/type"),
                                                null,
                                                false
                                        );
