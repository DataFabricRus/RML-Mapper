...
public class SubjectMapExtractor extends StdTermMapExtractor {
    ...
    // Log
    static final Logger log = LoggerFactory.getLogger(SubjectMapExtractor.class.getSimpleName());

    public SubjectMap extractSubjectMap(Repository repository,
        ...
        Set<FunctionTermMap> funObjectMaps;
        try {
            ...
            if (!funObjectMaps.isEmpty()) {
                result = funObjectMaps.toArray(new SubjectMap[1])[0];
                result.setOwnTriplesMap(triplesMap);
                result.setClassIRIs(classIRIs);
                return result;
            }
            ...
            try {
                if (connection.hasStatement(
                    ...
                    result = new StdConditionSubjectMap(
                            triplesMap,
                            constantValue,
                            stringTemplate,
                            termType,
